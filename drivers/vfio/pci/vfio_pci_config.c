...
/* 尝试读取前PCI物理设备配置空间的offset开始的size 个bytes的内容
 * 如果全部能读取，返回0
 *  如果读取失败，返回1
 * 注意： 这里只尝试测试物理设备配置空间指定位置的可访问性，没有具体修改配置处理
 */
				   ...
				   int offset, int size)
{
	...
	while (size) {
		...
		if (size >= 4 && !(offset % 4)) {
			...
			ret = pci_read_config_dword(pdev, offset, &dword); // 直接读取物理设备的配置空间对应offset
			if (ret)
				return ret;
...
int vfio_config_init(struct vfio_pci_device *vdev)
{
	...
	memset(map, PCI_CAP_ID_BASIC, PCI_STD_HEADER_SIZEOF); // 将map中的前64个字节设置为0
	memset(map + PCI_STD_HEADER_SIZEOF, PCI_CAP_ID_INVALID,
	       pdev->cfg_size - PCI_STD_HEADER_SIZEOF); // 将map中的256-64个字节的剩余空间设置为INVALID(FF)

	ret = vfio_fill_vconfig_bytes(vdev, 0, PCI_STD_HEADER_SIZEOF); //  测试物理设备配置空间的HEADER可读性
	if (ret)
		goto out;
				 ...
				 size_t count, loff_t *ppos, bool iswrite)
{
	...
	cap_id = vdev->pci_config_map[*ppos]; // ? vdev->pci_config_map[*ppos] 储存CAPID？
