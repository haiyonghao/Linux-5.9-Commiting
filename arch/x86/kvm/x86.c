...
/* 将正在等待的NMI计数器+1，并发出KVM_REQ_NMI请求*/
...
{
	++vcpu->stat.tlb_flush;
	kvm_x86_ops.tlb_flush_all(vcpu); // vmx_flush_tlb_all
}
...
/* CPU是否能够接收PIC或类似PIC(LVTINT0-EXTINT中断)的中断 */
...
/* 对于来自用户空间的set中断请求，分为2种情况：
  * 1. 如果lapic在用户空间，就直接将中断记录在vcpu->arch.interrupt,并req(EVENT)
  * 2. 如果lapic在kernel，就直接将中断记录在vcpu->arch.pending_external_vector中，并req(EVENT)
  */
				    ...
				    struct kvm_interrupt *irq)
{
	...
	if (!irqchip_in_kernel(vcpu->kvm)) { // 如果LAPIC在usesapce中
	 ...
	 */
	if (pic_in_kernel(vcpu->kvm)) // 如果lapic在kernel中，不使用kernel中的pic发送ExtINT
		return -ENXIO;

	if (vcpu->arch.pending_external_vector != -1)
		return -EEXIST;

	vcpu->arch.pending_external_vector = irq->irq; // 记录当前vCPU上正在等待处理的External vector
	kvm_make_request(KVM_REQ_EVENT, vcpu);
	return 0;
}
...
set_identity_unlock:
	...
	case KVM_CREATE_IRQCHIP: {
		...
		r = -EEXIST;
		if (irqchip_in_kernel(kvm)) // 创建irqchip时，不应该有irqchip已经存在，如果存在则直接跳过
			goto create_irqchip_unlock;

		r = -EINVAL;
		if (kvm->created_vcpus) // 创建irqchip时，不应该已经存在创建了的vcpus，否则直接跳过
			goto create_irqchip_unlock;
...
static void inject_pending_event(struct kvm_vcpu *vcpu, bool *req_immediate_exit)
{
	...
	if (vcpu->arch.exception.injected) { // exception.injected ==1 表示需要之前inject过，但现在需要重新inject
		kvm_x86_ops.queue_exception(vcpu); // vmx_queue_exception 向实模式或保护模式注入异常
		can_inject = false;
	}
	/*
	 ...
	 */
		...
		if (vcpu->arch.nmi_injected) { // nmi_injected表示允许尝试注入一个NMI
			kvm_x86_ops.set_nmi(vcpu); // vmx_inject_nmi 向实模式或保护模式注入NMI
			can_inject = false;
		} else if (vcpu->arch.interrupt.injected) {
			kvm_x86_ops.set_irq(vcpu); //  vmx_inject_irq 向实模式或保护模式注入Interrupt
			can_inject = false;
		}
	}
	...
	/* can_inject 这个变量，表示当前是否能够注入中断，一般情况下，如果
	 *  需要注入一些异常，那么从上面的代码中可以看到，均将can_inject 设置为0，
	 *  即当前无法注入中断。当这种情况发生时，进行注入中断会导致请求“immediate exit”，
	 *  “快速exit”会导致代码迅速退出，并迅速回到inject_pending_event中，
	 *  来处理导致can not inject 的其它event。
	 * */
	if (vcpu->arch.smi_pending) {
		r = can_inject ? kvm_x86_ops.smi_allowed(vcpu, true) : -EBUSY;  // vcpu在smm模式时，r=0，不在smm 模式，r=1
		if (r < 0)
			goto busy;
		if (r) { // 只要当前不在SMM 模式，就可以进入SMM模式；SMI的作用就是使CPU进入SMM模式
			vcpu->arch.smi_pending = false;
			++vcpu->arch.smi_count;
			enter_smm(vcpu);
			can_inject = false;
		} else 
			kvm_x86_ops.enable_smi_window(vcpu); // 本意是当CPU处于SMM模式时，尽快退出，但SMM模式自动退出时，会执行RSM指令，RSM本身就会导致vmexit，所以不需要特别操作
																									 // enable_smi_window函数内容为空，因为如果当前vcpu在smm模式，来的smi是无法处理的
	}
	...
	if (vcpu->arch.nmi_pending) {
		...
		if (r < 0)
			goto busy;
		if (r) { // 只要NMI没有被block，就可以设置NMI
			--vcpu->arch.nmi_pending;
			vcpu->arch.nmi_injected = true;
			kvm_x86_ops.set_nmi(vcpu); // vmx_inject_nmi 向实模式或保护模式注入NMI，如果vcpu在hlt，变为active状态
			can_inject = false;
	...
	if (kvm_cpu_has_injectable_intr(vcpu)) { // 不管是ExtINT，还是来自LAPIC的普通中断，都算是injectable_intr
		...
		if (r < 0)
			goto busy;
		if (r) { // 如果Guest允许中断
			kvm_queue_interrupt(vcpu, kvm_cpu_get_interrupt(vcpu), false); // 标记为硬中断+状态为已注入
			kvm_x86_ops.set_irq(vcpu); // vmx_inject_irq: 实模式模拟中断，其它模式写VMCS field 
			WARN_ON(kvm_x86_ops.interrupt_allowed(vcpu, true) < 0);
		}
		if (kvm_cpu_has_injectable_intr(vcpu)) // 如果注入中断后仍旧有injectable_intr，那就让vCPU在能处理中断时尽快退出
...
static int vcpu_enter_guest(struct kvm_vcpu *vcpu)
{
	...
	if (kvm_check_request(KVM_REQ_EVENT, vcpu) || req_int_win) { // req_int_win为1，表示CPU能接收来自PIC或类PIC(LVTINT0)中断
		++vcpu->stat.req_event;
		kvm_apic_accept_events(vcpu); // 对pending在apic->pending_events中的INIT或SIPI，进行处理
		if (vcpu->arch.mp_state == KVM_MP_STATE_INIT_RECEIVED) { // INIT信号被锁存了
			r = 1;
			goto out;
		}

		inject_pending_event(vcpu, &req_immediate_exit); // 检查是否有pending_event需要注入Guest,并进行注入动作
		if (req_int_win)
	...
	/* 如果请求了快速exit，就重新发起event请求，然后或设置vmx->req_immediate_exit
	 * 标志为true，或向vcpu所在的pcpu发送reschedule请求(当使能抢占时钟时)
	 */
...
/* 由于该函数会被consumer.add_producer实际调用，因此这里的prod，是在
 * producers链表中找到的与被操作的consumer的token相同的producer。 
 */
int kvm_arch_irq_bypass_add_producer(struct irq_bypass_consumer *cons,
				      struct irq_bypass_producer *prod)
{
	struct kvm_kernel_irqfd *irqfd =
		container_of(cons, struct kvm_kernel_irqfd, consumer); // irqfd->consumer
	int ret;

	irqfd->producer = prod;
	kvm_arch_start_assignment(irqfd->kvm);
	ret = kvm_x86_ops.update_pi_irte(irqfd->kvm, // 更新producer->irq的中断重定向 entry
					 prod->irq, irqfd->gsi, 1); // 其中host_irq为producer的irq，guest_irq为在qemu中通过ioctl注册的irqfd->gsi
