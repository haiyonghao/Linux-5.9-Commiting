...
struct kvm_vcpu_arch {
	...
	bool nmi_injected;    /* Trying to inject an NMI this entry  尝试在该entry中注入一个NMI*/
...
/* 该map用于记录flat model下的目标CPU mask
 * 也用于记录cluster model下的cluster编号和目标CPU mask 
 */
struct kvm_apic_map {
	struct rcu_head rcu;
	u8 mode;
...
#define __KVM_HAVE_ARCH_ASSIGNED_DEVICE
	...
	unsigned long apicv_inhibit_reasons;  // 32bit/64bit的"禁用APICV原因"变量  目前1-禁止；0-使能
...
/* 将irq_state的bit(irq_source_id)位置为0或1
 * 具体为0还是为1由level决定，0-0,1-1
 *  返回irq_state的bool值
 */
...
/* postable: 一个中断的deliver mode为Fixed或Lowest*/
